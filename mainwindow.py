...
class MqttClient(QThread):
    ...
    def run(self):
        ...
        else: self._ui.textBrowser.insertPlainText("VALUES DO NOT MATCH. CANNOT CONNECT- TRY AGAIN.\n")
    ...
    def on_connect(self, client, userdata, flags, rc):  # CALLBACK FUNC1
        ...
        else: self._ui.textBrowser.insertPlainText(f"FAILED TO CONNECT, RETURN CODE {rc}\n")

    def on_disconnect(self, client, userdata, rc, properties= None):  # CALLBACK FUNC2
...
class MainWindow(QMainWindow):
    ...
    def on_messageReceived(self, message, topic):  # 브로커로부터 메시지를 수신하면 호출된다
        ...
        if topic in self._mqtt_client.subscribed_topics and topic in self._mqtt_client.published_topics:
            self._ui.textBrowser.insertPlainText(f"MESSAGE RECEIVED FROM THE TOPIC YOU PUBLISHED: {current_date}\n TOPIC : {topic}\n MESSAGE: {data}\n")
        else:
            self._ui.textBrowser.insertPlainText(f"MESSAGE RECEIVED: {current_date} \n TOPIC: {topic}\n MESSAGE: {data}\n")
    ...
    def on_buttonConnect_clicked(self):
        if not self._mqtt_client.client.is_connected() == True:  # DISCONNECT FROM SERVER
            ...
            try:
                input_port = int(self._mqtt_client.port)
            except ValueError:
                ...
                run() 메소드에서 MQTT 클라이언트의 loop_start()를 호출해 MQTT 메시지 루프를 비동기적으로 실행한다.
            ...
            self._ui.tpc_txtEd.clear()
            self._ui.msg_txtEd.clear()
